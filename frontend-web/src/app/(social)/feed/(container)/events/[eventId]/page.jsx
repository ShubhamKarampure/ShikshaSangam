...
import MeetInvitationMessage from "./components/MeetInvitationMessage";   
...
const EventDetails = () => {
  ...
  const handleRegister = async() => {
    const response = await registerEvent({event:event.id,userprofile:profile.id})
    if(response){
        ...
        >
          ...
          <Row className="g-3 justify-content-between align-items-center mt-5 pt-5 position-relative z-index-9">
            ...
            <Col lg={3} className="text-lg-end">
              <Button variant="primary" onClick={handleRegister} disabled={disabled}>Register</Button>
            </Col>
          </Row>
        </Card>
            ...
            <Col xs={12}>
              ...
              {Object.entries(event.event_plan).map(([day, details]) => (
                <div key={day} className="mb-3">
                  <h6>{day}</h6>
                  {details.Time && (
                    <p className="small mb-1">Time: {details.Time}</p>
                  )}
                  {details.Activities && (
                  ...
                  {details.Mentorship && (
                    <p className="small mb-0">
                      Mentorship: {details.Mentorship}
                    </p>
                  )}
                  {details.VenueSetup && (
                    <p className="small mb-0">
                      Venue Setup: {details.VenueSetup}
                    </p>
                  )}
                  ...
                  {details.SafetySecurity && (
                    <p className="small mb-0">
                      Safety & Security: {details.SafetySecurity}
                    </p>
                  )}
                </div>
              ))}
