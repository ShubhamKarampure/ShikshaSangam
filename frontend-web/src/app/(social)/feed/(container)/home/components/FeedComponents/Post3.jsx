import { Card, CardBody, CardFooter, CardHeader, Dropdown, DropdownDivider, DropdownItem, DropdownMenu, DropdownToggle, Modal } from 'react-bootstrap';
...
const Post3 = (
  ...
  const handleComment = async (e,data)=>{
    ...
    setCommentsState([...commentsState,{comment:data,createdAt:(new Date()),socialUser:{avatar:`https://res.cloudinary.com/${cloudName}/${profile.avatar_image}`,name:username}}])
  }
  ...
  return (
    ...
    <Card>
      ...
      {image && <span role="button">
        <img src={image} alt="post-image" style={{width: '100%',height: '300px',objectFit: 'cover'}} onClick={()=>handleImageClick(image)}/>
      </span>}
      ...
      <CardFooter className="py-3">
        <ul className="nav nav-fill nav-stack small">
          <li className="nav-item">
            <Link className={`nav-link mb-0 ${like?"active":""}`} to="" onClick={handleLike}>
        ...
        {commentsState && <div className='px-4'>
            <div className="d-flex">
              <div className="avatar avatar-xs me-2">
                <span role="button">
                  
                  <img className="avatar-img rounded-circle" src={`https://res.cloudinary.com//${cloudName}/${profile.avatar_image}`} alt="avatar12" />
                </span>
              </div>

              <form className="nav nav-item w-100 position-relative" onSubmit={(e)=>handleComment(e,userComment)}>
                <textarea data-autoresize className="form-control pe-5 bg-light" rows={1} placeholder="Add a comment..." defaultValue={''} onChange={(e)=>setUserComment(e.target.value)} />
            ...
            <ul className="comment-wrap list-unstyled">
              {commentsState.map(comment => <CommentItem {...comment}  key={commentsState.comment}/>)}
            </ul>
          </div>}
